...
function createScanResponseData(name, serviceData) {
  i = 0;
  var scanDataLength = 0;
  var serviceDataBuf = null;
  if (name && name.length) {
    scanDataLength += 2 + name.length;
  }
  if (serviceData != null) {
      serviceDataBuf = new Buffer(serviceData[2], 'hex');
      scanDataLength += 4 + serviceDataBuf.length;
  }
  ...
  if (name && name.length) {
    ...
    if (serviceData != null) {
      scanData[i++] = serviceDataBuf.length + 3;
      scanData[i++] = 0x16;
      // The following 2 bytes specifiy what kind of service data this is
      // E.g. 0x0A followed by 0x18 is "Device Information"
      scanData[i++] = serviceData[0];
      scanData[i++] = serviceData[1];
      for (j = 0; j < serviceDataBuf.length; j++) {
        scanData[i++] = serviceDataBuf[j];
      }
    }
  }
  return scanData;
...
HciBle.prototype.startAdvertising = function(name, serviceUuids) {
  ...
  var scanData = createScanResponseData(name, null);
  this.startAdvertisingWithEIRData(advertisementData, scanData);
};

HciBle.prototype.startAdvertisingAndIBeacon = function(name, serviceData, data) {
  var advertisementData = createIBeaconAdvertisementData(data);
  var scanData =  createScanResponseData(name, serviceData);
  this.startAdvertisingWithEIRData(advertisementData, scanData);
};

function createIBeaconAdvertisementData(data) {
